...
def visual(true, preds=None, aug=None, name='./pic/test.pdf'):
    """
    ...
    if preds is not None:
        if aug is not None:
            plt.plot(preds, label='Augmentation_Prediction', c='dodgerblue', linewidth=2)
        else:
            plt.plot(preds, label='Prediction', c='dodgerblue', linewidth=2)
    if aug is not None:
        plt.plot(aug, label='Augmentation_GroundTruth', c='green', linewidth=2)
